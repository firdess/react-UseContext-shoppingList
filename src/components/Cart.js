...
const Cart = () => {
  ...
  const totalPrice = context.state.cart
    .reduce((total, book) => total + book.price * book.count, 0)
    .toFixed(2);

  const totalBook = context.state.cart.reduce(
  ...
  return (
    <div>
      <h2>
        <Link to="/">Kitap Listesi</Link> <span>Sepetim ({totalBook})</span>
      </h2>
      ...
      {context.state.cart.map((book) => (
        <div className="book" key={book.id}>
          ...
          <div>
            ...
            <button onClick={() => context.removeItem(book.id)}>
              Sepetten Çıkar
            </button>
